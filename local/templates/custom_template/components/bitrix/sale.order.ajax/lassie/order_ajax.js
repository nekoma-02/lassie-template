	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActiveBasketBlock: function (activeNodeMode) {
			...
			if (this.initialized.basket) {
				...
				basketContent.appendChild(
					BX.create('DIV', {
						props: { className: 'bx-soa-table-fade goods-table' },
						children: [
							BX.create('DIV', {
		...
		editBasketItemsHeader: function (basketItemsNode) {
			if (!basketItemsNode)
				...
				var headers = [
					BX.create('DIV', {
						props: {className: 'bx-soa-item-td goods-table__heading'},
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				headers.push(
					BX.create('DIV', {
						props: {className: 'bx-soa-item-td bx-soa-item-properties goods-table__heading' + (toRight ? ' bx-text-right' : '')},
			...
			// headerChild = BX.create('DIV', {
			// 	props: { className: 'bx-soa-item-tr goods-table__header-row hidden-sm hidden-xs' },
			// 	children: headers
			// });
		...
		createBasketItem: function (basketItemsNode, item, index, active) {
			...
			cols = [
				BX.create('DIV', {
					props: { className: 'bx-soa-item-td goods-table__cell' },
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: { className: 'bx-soa-item-tr bx-soa-basket-info goods-table__row' + (index == 0 ? ' bx-soa-item-tr-first' : '') },
					children: cols
				})
			);
		...
		createBasketItemImg: function (data) {
			...
			var divImgBlock = BX.create('DIV',{props: {className: 'goods-table__col'}});

			logoNode = BX.create('IMG', { props: { className: 'bx-soa-item-imgcontainer goods-table__img' } });
			...
			if (this.params.HIDE_DETAIL_PAGE_URL !== 'Y' && data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length) {
				logoNode = BX.create('A', {
					props: { src: data.DETAIL_PAGE_URL },
					children: [logoNode]
				});
			}
			divImgBlock.appendChild(BX.create('DIV', {
				props: { className: 'bx-soa-item-img-block goods-table__img-wrapper' },
				children: [logoNode]
			}));
			return divImgBlock;
		},
		...
		createBasketItemContent: function (data) {
			...
			if (this.options.showPropsInBasket && props.length) {
				for (var i in props) {
					if (props.hasOwnProperty(i)) {
						...
						propsNodes.push(
							BX.create('DIV', {
								props: { className: 'bx-soa-item-td-text goods-table__info' },
			...
			var divContent = BX.create('DIV',{props: {className: 'goods-table__col'}});
			divContent.appendChild(BX.create('DIV', {
				props: { className: 'bx-soa-item-content' },
				children: propsNodes.length ? [
					BX.create('DIV', { props: { className: 'bx-soa-item-title goods-table__text' }, html: titleHtml }),
		...
		createBasketItemColumn: function (column, allData, active) {
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				toRight = BX.util.in_array(column.id, ["QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"]),
				textNode = BX.create('DIV', { props: { className: 'bx-soa-item-td-text goods-table__text' } }),
				logotype, img;
			...
			return BX.create('DIV', {
				props: { className: 'bx-soa-item-td bx-soa-item-properties goods-table__cell' + (toRight ? ' bx-text-right' : '') },
				children: [textNode]
		...
		getDeliveryLocationInput: function (node) {
			...
			if (altProperty) {
				altNode = BX.create('DIV', {
					...
					props: { className: "form-group bx-soa-location-input-container form__row" }
				});
				...
				label = BX.create('LABEL', {
					...
					props: { className: 'bx-soa-custom-label form__label' },
					html: labelTextHtml,
					for: 'altProperty',
				});
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'form-control bx-soa-customer-input bx-ios-fix input',
		...
		getZipLocationInput: function (node) {
			...
			if (zipProperty) {
				...
				propsItemNode = BX.create('DIV', { props: { className: "form-group bx-soa-location-input-container form__row" } });
				...
				divLabel = BX.create('DIV', {
					props: {
						className: 'form__col form__col_width_220'
					}
				});
				divInput = BX.create('DIV', {
					props: {
						className: 'form__col form__col_width_260'
					}
				});
				...
				propsItemNode.appendChild(divLabel);
				propsItemNode.appendChild(divInput);
		...
		createPaySystemItem: function (item) {
			...
			label = BX.create('input', {
				props: {
					id: 'ID_PAY_SYSTEM_ID_' + paySystemId,
					name: 'PAY_SYSTEM_ID',
					type: 'checkbox',
					className: 'bx-soa-pp-company-checkbox radio__elem',
					value: paySystemId,
					checked: checked
				}
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				title = BX.create('label', 
				{ 
					props: { className: 'bx-soa-pp-company-smalltitle form__label radio__label' }, 
					text: item.NAME,
			...
			itemNode = BX.create('DIV', {
				props: { className: 'bx-soa-pp-company radio' },
		...
		createDeliveryItem: function (item) {
				...
				labelNodes = [
					BX.create('INPUT', {
						props: {
							...
							name: 'DELIVERY_ID',
							type: 'checkbox',
							className: 'bx-soa-pp-company-checkbox checkbox_custom_elem',
							value: deliveryId,
							checked: checked
						}
					})
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
				title = BX.create('LABEL', {
					props: { className: 'bx-soa-pp-company-smalltitle form__label' },
			...
			itemNode = BX.create('DIV', {
				props: { className: 'bx-soa-pp-company form__checkbox-group' },
				children: [label],
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined') {
				itemNode.appendChild(
					BX.create('DIV', {
						props: { className: 'bx-soa-pp-delivery-cost form__label' },
		...
		getPropertyRowNode: function (property, propsItemsContainer, disabled) {
			...
			if (disabled) {
				...
				BX.addClass(propsItemNode, "form-group bx-soa-customer-field form__row");
				...
				div = BX.create('DIV', { props: { className: 'form__col form__col_width_220' } });
		...
		insertDateProperty: function (property, propsItemNode, disabled) {
			...
			if (disabled) {
				...
				propContainer = BX.create('DIV', { props: { className: 'soa-property-container form__col' } });
		...
		alterProperty: function (settings, propContainer) {
			...
			if (add.length) {
				...
				BX.bind(add, 'click', BX.delegate(function (e) {
					var target = e.target || e.srcElement,
						targetContainer = BX.findParent(target, { tagName: 'div', className: 'soa-property-container form__col' }),
					...
					if (textInputs.length) {
						...
						BX.addClass(textInputs[textInputs.length - 1], 'form-control bx-soa-customer-input bx-ios-fix input');
		...
		isValidPropertiesBlock: function (excludeLocation) {
			...
			for (i = 0; i < props.length; i++) {
				...
				propContainer = props[i].querySelector('.soa-property-container form__col');
				if (propContainer) {
					arProperty = this.validation.properties[id];
		...
		editPropsComment: function (propsNode) {
			...
			div = BX.create('DIV', {
				props: { className: 'form-group bx-soa-customer-field form__row' },
		...
		editTotalBlock: function () {
			...
			this.totalInfoBlockNode.appendChild(
				BX.create('P', {text: 'Заполнение вышеуказанных данных является необходимым для оформления Заказа и осуществления доставки. Все данные находятся на территории РФ, согласно Законодательству РФ.'})
			);

			var textSoglashenie = BX.create('A',{props:{href: 'javascript:void(0);', className: 'link'}, text: 'Пользовательскому Соглашению.'});

			var pTagSoglashenie = BX.create('P', {text: 'Предоставляя свои персональные данные при оформлении Заказа, подтверждаю свое согласие на обработку моих персональных данных, а также на аудиозапись своего общения с представителями Сайта в период оформления Заказа и исполнения обязательств согласно ',});
			
			pTagSoglashenie.appendChild(textSoglashenie);
		...
		createTotalUnit: function (name, value, params) {
			var totalValue, className = 'bx-soa-cart-total-line form__total-row';
			...
			if (params.total) {
				className += ' bx-soa-cart-total-line-total form__total-row_general';
			}
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('SPAN', { props: { className: 'bx-soa-cart-t form__total-item' }, text: name }),
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-cart-d form__total-item' + (!!params.total && this.options.totalPriceChanged ? ' bx-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
