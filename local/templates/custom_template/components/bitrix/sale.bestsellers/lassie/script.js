	...
	window.JCCatalogSectionBest.prototype.QuantitySet = function(index)
	{
		if (0 === this.errorCode)
		{
			...
			if (this.canBuy)
			{
				...
				this.obBuyBtn.disabled = '';
			}
			else
			{
				BX.addClass(this.obBuyBtn, 'bx_bt_button_type_2');
				BX.removeClass(this.obBuyBtn, 'bx_bt_button');
				this.obBuyBtn.disabled = 'disabled';
			...
			if (!!RowItems && 0 < RowItems.length)
			{
				...
				for (i = 0; i < RowItems.length; i++)
				{
					...
					if (BX.util.in_array(value, canBuyID))
					{
						...
						obData.props.className = (isCurrent ? 'bx_active checkbox-tile__elem' : 'checkbox-tile__elem');
						obData.props.checked = (isCurrent ? 'checked' : '');
					}
					else
					{
					...
					BX.adjust(RowItems[i].parentNode, obData2);
				}
				...
				BX.adjust(this.obTreeRows[intNumber].CONT, {props: {className: 'good__order-row'}});
				if (extShowMode)
				{
	...
	window.JCCatalogSectionBest.prototype.ChangeInfo = function()
	{
		...
		if (-1 < index)
		{
			if (!!this.obPict)
			{
				...
				{
					BX.adjust(this.obPict.firstChild, {src: this.offers[index].PREVIEW_PICTURE.SRC});
				}
				else
				{
					BX.adjust(this.obPict.firstChild, {src: this.defaultPict.pict.SRC});
				}
			}
			...
			if (!!this.obPrice)
			{
				strPrice = '<span class="good__price">'+this.offers[index].PRICE.PRINT_DISCOUNT_VALUE+'</span>';
				if (this.showOldPrice && (this.offers[index].PRICE.DISCOUNT_VALUE !== this.offers[index].PRICE.VALUE))
				{
					strPrice = 
					'<span class="good__price good__price_new">'+this.offers[index].PRICE.PRINT_DISCOUNT_VALUE+'</span>'+
					'<span class="good__price good__price_old">'+this.offers[index].PRICE.PRINT_VALUE+'</span>';
				}
