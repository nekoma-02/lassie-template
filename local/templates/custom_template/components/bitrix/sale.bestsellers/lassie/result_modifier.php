	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($propertyName && isset($arItem['PROPERTIES'][$propertyName]))
		{
			...
			if (!empty($property['VALUE']))
			{
				...
				{
					$arItem['LABEL_VALUE'] = $property['CODE'];
				}
				else
				{
		...
		if(SiteCurrencyTable::getSiteCurrency(SITE_ID) == "RUB" && LANGUAGE_ID === "ru"){
			$arChangePrice = [
				'PRINT_DISCOUNT_VALUE',
				'PRINT_VALUE',
				'PRINT_DISCOUNT_DIFF'
			];
			$arChangeVar = [ &$arItem['MIN_PRICE'] ];
			foreach($arItem['JS_OFFERS'] as $keyCangeOffer => $notNeedVal){
				$arChangeVar[] = &$arItem['JS_OFFERS'][$keyCangeOffer]['PRICE'];
			}
			foreach ($arChangeVar as &$changeVar) {
				foreach ($arChangePrice as $str) {
